...
export default function CartPage() {
  ...
  const loadCart = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const cartWithDetails = data.items.map((item: any) => {
          ...
          return {
            ...
            image: product?.images[0] || item.pictureUrl || '',
            size: item.size,
            color: item.color,
            quantity: item.quantity,
            category: product?.category || item.productType || '',
            sku: product?.sku || '',
  ...
  const removeItem = async (cartItemId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/cart/remove?productId=${item.productId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
